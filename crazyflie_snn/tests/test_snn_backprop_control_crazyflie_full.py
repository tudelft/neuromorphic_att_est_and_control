...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="configs/new/bp_snn_control_crazyflie_full_combined.yaml")
    ...
    if config["net"]["checkpoint"]:
        model_path_rate = f"runs/{config['net']['checkpoint_rate']}/model.pt"
        model_path_torque = f"runs/{config['net']['checkpoint_torque']}/model.pt"
        model.load_combined_state_dict(torch.load(model_path_rate), torch.load(model_path_torque))
    ...
    run = wandb.init(project="snn_control", entity="sstroobants", config=config, mode=wandb_mode, settings=wandb.Settings(_service_wait=300))
